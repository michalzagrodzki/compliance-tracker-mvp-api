...
class AuditLogService:
    ...
    async def get_audit_log_statistics(self, user_id: str, compliance_domain: Optional[str] = None) -> AuditLogStatistics:
        """Get audit log statistics with access control."""
        try:
            # Validate user access
            user = await self.user_repository.get_by_id(user_id)
            if not user:
            ...
            if compliance_domain and not (user.is_admin() or user.can_access_domain(compliance_domain)):
                raise AuthorizationException(
                    detail=f"Access denied to audit statistics for compliance domain: {compliance_domain}",
                    error_code="DOMAIN_ACCESS_DENIED"
                )
            
            return await self.audit_log_repository.get_statistics(compliance_domain)
    ...
    async def delete_audit_log(self, log_id: str, user_id: str) -> bool:
        """Delete an audit log entry. Note: This should be used sparingly as audit logs are meant to be immutable."""
        try:
            ...
            if not user.is_admin():
                raise AuthorizationException(
                    detail="Only administrators can delete audit log entries",
                    error_code="ADMIN_ACCESS_REQUIRED"
                )
            
            # Get audit log for logging before deletion
            existing_log = await self.audit_log_repository.get_by_id(log_id)
            if not existing_log:
                raise ResourceNotFoundException(
                    resource_type="AuditLog",
                    resource_id=log_id
            ...
            success = await self.audit_log_repository.delete(log_id)
            
            if success:
                # Log the deletion (meta-audit log)
                log_business_event(
                    ...
                    user_id=user_id,
                    details={
                        "deleted_log_object_type": existing_log.object_type,
                        "deleted_log_object_id": existing_log.object_id,
                        "deleted_log_action": existing_log.action
                    }
                )
